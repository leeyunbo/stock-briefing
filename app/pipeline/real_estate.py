...
Stage 1 — 데이터 수집 (병렬)
  ├── fetch_real_estate_news()   → 네이버 뉴스 API (부동산 키워드 5개)
  └── fetch_all_subscription_data() → 청약Home API (접수중/접수예정)
Stage 2 — 리포트 생성 (Claude)
  └── 뉴스 + 청약 데이터 → HTML 부동산 브리핑

→ _deliver(result, "real_estate_briefing", email_to)
"""
...
from app.pipeline.research import _deliver
...
def generate_real_estate_report(data: RealEstateData) -> BriefingResult:
    ...
    title = seo_title or f"{date.today().strftime('%Y년 %m월 %d일')} 부동산 브리핑"
...
async def run_real_estate_pipeline(email_to: list[str] | None = None) -> str:
    ...
    Stage 1: 뉴스 + 청약 데이터 병렬 수집
    Stage 2: Claude 리포트 생성
    → 저장 + WordPress 발행 + 이메일 발송
    """
    ...
    result = generate_real_estate_report(data)

    # 저장 + 발송
    await _deliver(result, briefing_type="real_estate_briefing", email_to=email_to)
