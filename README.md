...
매일 6개 파이프라인이 자동 운영되며, 미국주식·한국주식·부동산·이슈 분석 기사를 WordPress 블로그에 발행하고 구독자에게 이메일로 전달합니다.
...
| **이슈 딥다이브** | 매일 10·15·21시 | 가장 중요한 이슈 1개를 6개 섹션으로 깊게 분석하는 피처 기사 |
...
│              │     │              │     │              │     │             │
│ Finnhub      │     │ ChromaDB     │     │ Claude CLI   │     │ DB 저장     │
│ yfinance     │     │ Gemini       │     │ SEO 메타     │     │ WordPress   │
│ 네이버 뉴스   │     │ Embedding    │     │ 추출         │     │ 이메일 발송  │
│ DART 공시    │     │              │     │              │     │ Google 색인  │
│ 청약홈       │     │              │     │              │     │ Unsplash    │
└─────────────┘     └──────────────┘     └──────────────┘     └─────────────┘
```
...
```
collect → dedup → LLM #1: 이슈 선정 → 추가 리서치 → LLM #2: 기사 작성 → 발행
                  (가장 중요한       (키워드 뉴스     (6개 섹션
                   이슈 1개 선정)     + 종목 스크리닝)  장문 피처 기사)
```
...
| AI | Claude CLI, Gemini Embedding |
| 데이터 수집 | Finnhub, yfinance, 네이버 뉴스, DART, 청약홈 |
...
├── core/               # 설정, DB, 모델
├── collector/           # 데이터 수집 모듈
│   ├── market_scan.py   #   나스닥 100 스캔 + 매크로 지표
│   ├── market.py        #   네이버 금융 (코스피/코스닥)
│   ├── news.py          #   네이버 뉴스 RSS
│   ├── dart.py          #   DART 공시
│   ├── real_estate.py   #   청약홈 + 부동산 뉴스
│   ├── dedup.py         #   ChromaDB 시맨틱 중복 제거
│   └── stock_research.py#   Finnhub 종목 스크리닝
├── prompts/             # LLM 프롬프트 + 호출
│   ├── nasdaq.py        #   나스닥 브리핑
│   ├── news_dive.py     #   뉴스 딥다이브
│   ├── issue_dive.py    #   이슈 딥다이브 (2단계)
│   ├── real_estate.py   #   부동산 브리핑
│   └── deep_research.py #   개별 종목 딥리서치
├── pipeline/            # 파이프라인 오케스트레이션
│   ├── base.py          #   공통 스텝 러너 + Publisher 패턴
│   ├── nasdaq.py        #   나스닥 파이프라인
│   ├── kospi.py         #   코스피 파이프라인
│   ├── issue_dive.py    #   이슈 딥다이브 파이프라인
│   ├── real_estate.py   #   부동산 파이프라인
│   └── digest.py        #   데일리 다이제스트
├── publishing/          # 발행 모듈
